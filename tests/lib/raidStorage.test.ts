...
describe("normalizeRaidSlots, raid storage", () => {
  it("returns a fresh set of empty raid slots for non-object input", () => {
    expect(normalizeRaidSlots(null)).toEqual(createInitialRaidSlots());
    expect(normalizeRaidSlots("bad data")).toEqual(createInitialRaidSlots());
  });
  ...
  it("keeps valid placed specs, normalizes invalid and null slots to empty", () => {
    ...
    expect(result["1-2"]).toEqual({ classId: "druid", specId: "balance" });
  ...
  it("drops malformed slot values back to null", () => {
    ...
    expect(result["1-3"]).toEqual({ classId: "druid", specId: "balance" });
  });
